...
namespace Phongkham.Areas.Patient.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> IndexLK()
        {
            ...
            // Lấy danh sách lịch khám cho nha sĩ hiện tại
            var lichKhams = await _context.cTlichkhams
                .Where(ct => ct.TenBenhNhan == currentUser.FullName) // So sánh với tên nha sĩ
                .Select(ct => new
                {
                    ct.TenNhaSi,
                    ct.PhongKham,
                    LichKhamPhone = ct.LichKham.phone, // Truy cập thuộc tính phone từ LichKham
